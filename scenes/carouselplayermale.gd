...
func _ready() -> void:
	...
	if FileAccess.file_exists("user://selected_player.save"):
		...
		file.close()
		card_current_index = selected_player_index1  # Set carousel position
		scroll()
...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed("ui_right") and selected_player_index1< card_nodes.size() - 1:
		selected_player_index1 += 1
		card_current_index = selected_player_index1 # Ensure correct index
		print("Moving Right to Index:", selected_player_index1)
		scroll()
	elif event.is_action_pressed("ui_left") and selected_player_index1 > 0:
		selected_player_index1 -= 1
		card_current_index = selected_player_index1  # Ensure correct index
		print("Moving Left to Index:", selected_player_index1)
		scroll()
...
func select_player():
	GameController.selected_player_index1= card_current_index
